<?php
	...
	$total=sql_fetch("select count(*) as cnt `{$g5['point_table']}` as p inner join `g5_member` as m on p.mb_id=m.mb_id where m.mb_level = '1'");
	if(!$page)
		$page=1;
	$total=$total['cnt'];
	$rows=10;
    ...
    $sql="select p.*,m.mb_level,m.mb_name,m.mb_no from `{$g5['point_table']}` as p inner join `g5_member` as m on p.mb_id=m.mb_id where m.mb_level = '1' {$where} order by `po_id` desc limit {$start},{$rows}";
    $query=sql_query($sql);
    $j=0;
		...
		<div class="grid_100" style="margin-bottom:30px">
				<form action="" method="get">
					<div class="grid_25">
						<select name="sel" id="sel" class="grid_100 adm-input01">
                            ...
                            <option value="p.po_datetime" <?php echo $sel=="p.po_datetime"?"selected":""; ?>>적립일</option>
						</select>
					</div>
			...
			<div class="adm-table01">
				...
				<div class="text-right mt20">
                        <a href="<?php echo G5_URL."/admin/point_write.php?page=".$page; ?>" class="adm-btn01">추가하기</a>
                        <a href="<?php echo G5_URL."/admin/excel_member.php?sel=".$sel."&search=".$search; ?>" class="adm-btn01">엑셀로저장</a>
                </div>				  
			</div>
			<?php
				if($total_page>1){
			...
			<div class="num_list01">
				<ul>
				<?php if($page!=1){?>
					<li class="prev"><a href="<?php echo G5_URL."/admin/point.php?page=".($page-1); ?>">&lt;</a></li>
				<?php } ?>
				<?php for($i=$start_page;$i<=$end_page;$i++){ ?>
					<li class="<?php echo $page==$i?"active":""; ?>"><a href="<?php echo G5_URL."/admin/point.php?page=".$i; ?>"><?php echo $i; ?></a></li>
				<?php } ?>
				<?php if($page<$total_page){?>
					<li class="next"><a href="<?php echo G5_URL."/admin/point.php?page=".($page+1); ?>">&gt;</a></li>
				<?php } ?>
				</ul>
			</div>
			<?php
