...
?>
    <style>
        ...
        .store_menu_list{background:#ddd;padding:10px;}
    ...
    <div class="width-fixed view">
        <form id="fregisterform" name="fregisterform" action="<?php echo G5_URL."/page/shop/my_store_add_update.php";?>" onsubmit="return fregisterform_submit(this);" method="post" enctype="multipart/form-data" autocomplete="off">
            <div class="store_menu_title">
                <div><h2 class="detail_title">메뉴분류</h2></div>
            </div>
                ...
                <dl>
                    ...
                    if(count($view) != 0) {
                        for ($i = 0; $i < count($view); $i++) {
                            ...
                            ?>
                            <dd class="store_menu <?php if($i==0){?>first<?php }?>" ><?php echo $view[$i]["ca_name"]; ?> - <?php echo $count["cnt"]?> 개 <a href="<?php echo G5_URL?>/page/shop/my_store_menu_2depth_edit.php?ca_name=<?php echo $view[$i]["ca_name"];?>&wr_id=<?=$wr_id?>&ca_id=<?php echo $view[$i]["id"]?>"><img src="<?php echo G5_IMG_URL?>/mypage_edit.png" class="menu_list_edit"></a></dd>
                        <?php }
                    }else{ ?>
